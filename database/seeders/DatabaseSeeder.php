    ...
    public function run(): void
    {
        ...
        foreach ($classes as $classe) {
            ...
            foreach ($subjects as $subject) {
                // Créer une nouvelle entrée d'emploi du temps pour chaque matière dans chaque classe
                \App\Models\TimeTable::create([
                    'academic_year_id' => 1, // Définir l'année académique
                    'classe_id' => $classe->id, // Définir l'ID de la classe
                    'subject_id' => $subject->id, // Définir l'ID de la matière
                    'period_id' => 1, // Définir l'ID de la période
                    'teacher_id' => rand(1, 30), // Assigner un enseignant aléatoire
                    'coefficient' => rand(1, 6), // Assigner un coefficient aléatoire
                    'day' => ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'][rand(0, 6)], // Assigner un jour aléatoire
                    'start_time' => now()->startOfDay()->addHours(rand(8, 15)), // Assigner une heure de début aléatoire
                    'end_time' => now()->startOfDay()->addHours(rand(16, 20)), // Assigner une heure de fin aléatoire
                ]);
            }
        }
